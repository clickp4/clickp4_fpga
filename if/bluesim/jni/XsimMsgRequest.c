...
int XsimMsgRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_XsimMsgRequest_msgSink: {
        ...
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.msgSink.data = (uint32_t)(((tmp)&0xfffffffful));((XsimMsgRequestCb *)p->cb)->msgSink(p, tempdata.msgSink.portal, tempdata.msgSink.data);
      } break;
    case CHAN_NUM_XsimMsgRequest_msgSinkFd: {
        ...
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.msgSinkFd.data = messageFd;((XsimMsgRequestCb *)p->cb)->msgSinkFd(p, tempdata.msgSinkFd.portal, tempdata.msgSinkFd.data);
      } break;
    default:
