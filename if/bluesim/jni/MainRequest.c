...
int MainRequest_module_if_if_equal_add_entry ( struct PortalInternal *p, const ModuleIfIfEqualReqT key, const ModuleIfIfEqualRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_if_if_equal_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_if_if_equal_add_entry, "MainRequest_module_if_if_equal_add_entry")) return 1;
    ...
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_if_if_equal_add_entry << 16) | 5, -1);
    return 0;
};

int MainRequest_module_if_if_large_add_entry ( struct PortalInternal *p, const ModuleIfIfLargeReqT key, const ModuleIfIfLargeRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_if_if_large_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_if_if_large_add_entry, "MainRequest_module_if_if_large_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.click_id>>23)|(((unsigned long)key.padding)<<9));
    p->transport->write(p, &temp_working_addr, val._state|(((unsigned long)val._action)<<8)|(((unsigned long)key.click_id)<<9));
    p->transport->write(p, &temp_working_addr, (val._bitmap>>32));
    p->transport->write(p, &temp_working_addr, val._bitmap);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_if_if_large_add_entry << 16) | 5, -1);
    return 0;
};

int MainRequest_module_if_if_small_add_entry ( struct PortalInternal *p, const ModuleIfIfSmallReqT key, const ModuleIfIfSmallRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_if_if_small_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_if_if_small_add_entry, "MainRequest_module_if_if_small_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.click_id>>23)|(((unsigned long)key.padding)<<9));
    p->transport->write(p, &temp_working_addr, val._state|(((unsigned long)val._action)<<8)|(((unsigned long)key.click_id)<<9));
    p->transport->write(p, &temp_working_addr, (val._bitmap>>32));
    p->transport->write(p, &temp_working_addr, val._bitmap);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_if_if_small_add_entry << 16) | 5, -1);
    return 0;
};
...
MainRequestCb MainRequestProxyReq = {
    ...
    MainRequest_module_if_if_equal_add_entry,
    MainRequest_module_if_if_large_add_entry,
    MainRequest_module_if_if_small_add_entry,
};
...
int MainRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_module_if_if_equal_add_entry: {
        ...
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_equal_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1fful))<<23));
        tempdata.module_if_if_equal_add_entry.key.padding = (uint8_t)(((tmp>>9)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_equal_add_entry.val._state = (uint8_t)(((tmp)&0xfful));
        tempdata.module_if_if_equal_add_entry.val._action = (uint8_t)(((tmp>>8)&0x1ul));
        tempdata.module_if_if_equal_add_entry.key.click_id |= (uint32_t)(((tmp>>9)&0x7ffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_equal_add_entry.val._bitmap = (uint64_t)(((uint64_t)(((tmp)&0xfffffffful))<<32));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_equal_add_entry.val._bitmap |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_if_if_equal_add_entry(p, tempdata.module_if_if_equal_add_entry.key, tempdata.module_if_if_equal_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_if_if_large_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_large_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1fful))<<23));
        tempdata.module_if_if_large_add_entry.key.padding = (uint8_t)(((tmp>>9)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_large_add_entry.val._state = (uint8_t)(((tmp)&0xfful));
        tempdata.module_if_if_large_add_entry.val._action = (uint8_t)(((tmp>>8)&0x1ul));
        tempdata.module_if_if_large_add_entry.key.click_id |= (uint32_t)(((tmp>>9)&0x7ffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_large_add_entry.val._bitmap = (uint64_t)(((uint64_t)(((tmp)&0xfffffffful))<<32));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_large_add_entry.val._bitmap |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_if_if_large_add_entry(p, tempdata.module_if_if_large_add_entry.key, tempdata.module_if_if_large_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_if_if_small_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_small_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1fful))<<23));
        tempdata.module_if_if_small_add_entry.key.padding = (uint8_t)(((tmp>>9)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_small_add_entry.val._state = (uint8_t)(((tmp)&0xfful));
        tempdata.module_if_if_small_add_entry.val._action = (uint8_t)(((tmp>>8)&0x1ul));
        tempdata.module_if_if_small_add_entry.key.click_id |= (uint32_t)(((tmp>>9)&0x7ffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_small_add_entry.val._bitmap = (uint64_t)(((uint64_t)(((tmp)&0xfffffffful))<<32));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_if_if_small_add_entry.val._bitmap |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_if_if_small_add_entry(p, tempdata.module_if_if_small_add_entry.key, tempdata.module_if_if_small_add_entry.val);
      } break;
    default:
