...
int MainRequest_module_for_for_end_add_entry ( struct PortalInternal *p, const ModuleForForEndReqT key, const ModuleForForEndRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_for_for_end_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_for_for_end_add_entry, "MainRequest_module_for_for_end_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.click_id>>23)|(((unsigned long)key.padding)<<9));
    p->transport->write(p, &temp_working_addr, val._state|(((unsigned long)val._action)<<8)|(((unsigned long)key.click_id)<<9));
    p->transport->write(p, &temp_working_addr, (val._bitmap>>32));
    p->transport->write(p, &temp_working_addr, val._bitmap);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_for_for_end_add_entry << 16) | 5, -1);
    return 0;
};

int MainRequest_module_for_for_init_add_entry ( struct PortalInternal *p, const ModuleForForInitReqT key, const ModuleForForInitRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_for_for_init_add_entry, 4);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_for_for_init_add_entry, "MainRequest_module_for_for_init_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.click_id>>31)|(((unsigned long)key.padding)<<1));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.click_id)<<1));
    p->transport->write(p, &temp_working_addr, val._threshold);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_for_for_init_add_entry << 16) | 4, -1);
    return 0;
};

int MainRequest_module_for_for_loop_add_entry ( struct PortalInternal *p, const ModuleForForLoopReqT key, const ModuleForForLoopRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_for_for_loop_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_for_for_loop_add_entry, "MainRequest_module_for_for_loop_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.click_id>>31)|(((unsigned long)key.padding)<<1));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.click_id)<<1));
    p->transport->write(p, &temp_working_addr, (val._bitmap>>32));
    p->transport->write(p, &temp_working_addr, val._bitmap);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_for_for_loop_add_entry << 16) | 5, -1);
    return 0;
};
...
MainRequestCb MainRequestProxyReq = {
    ...
    MainRequest_module_for_for_end_add_entry,
    MainRequest_module_for_for_init_add_entry,
    MainRequest_module_for_for_loop_add_entry,
};
...
{
    return "{\"read_pktcap_perf_info\": [], \"read_version\": [], \"module_for_for_init_add_entry\": [\"long\", \"long\"], \"writeMetaGenData\": [\"long\", \"long\", \"long\", \"long\"], \"metagen_start\": [\"long\", \"long\"], \"writePktGenData\": [\"long\", \"long\", \"long\", \"long\"], \"writePacketData\": [\"long\", \"long\", \"long\", \"long\"], \"pktcap_stop\": [], \"pktcap_start\": [\"long\"], \"pktgen_stop\": [], \"module_for_for_loop_add_entry\": [\"long\", \"long\"], \"set_verbosity\": [\"long\"], \"pktgen_start\": [\"long\", \"long\", \"long\"], \"metagen_stop\": [], \"module_for_for_end_add_entry\": [\"long\", \"long\"]}";
}
...
int MainRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_module_for_for_end_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_end_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1fful))<<23));
        tempdata.module_for_for_end_add_entry.key.padding = (uint8_t)(((tmp>>9)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_end_add_entry.val._state = (uint8_t)(((tmp)&0xfful));
        tempdata.module_for_for_end_add_entry.val._action = (uint8_t)(((tmp>>8)&0x1ul));
        tempdata.module_for_for_end_add_entry.key.click_id |= (uint32_t)(((tmp>>9)&0x7ffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_end_add_entry.val._bitmap = (uint64_t)(((uint64_t)(((tmp)&0xfffffffful))<<32));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_end_add_entry.val._bitmap |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_for_for_end_add_entry(p, tempdata.module_for_for_end_add_entry.key, tempdata.module_for_for_end_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_for_for_init_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 3, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_init_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1ul))<<31));
        tempdata.module_for_for_init_add_entry.key.padding = (uint8_t)(((tmp>>1)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_init_add_entry.val._action = (uint8_t)(((tmp)&0x1ul));
        tempdata.module_for_for_init_add_entry.key.click_id |= (uint32_t)(((tmp>>1)&0x7ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_init_add_entry.val._threshold = (uint32_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_for_for_init_add_entry(p, tempdata.module_for_for_init_add_entry.key, tempdata.module_for_for_init_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_for_for_loop_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_loop_add_entry.key.click_id = (uint32_t)(((uint32_t)(((tmp)&0x1ul))<<31));
        tempdata.module_for_for_loop_add_entry.key.padding = (uint8_t)(((tmp>>1)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_loop_add_entry.val._action = (uint8_t)(((tmp)&0x1ul));
        tempdata.module_for_for_loop_add_entry.key.click_id |= (uint32_t)(((tmp>>1)&0x7ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_loop_add_entry.val._bitmap = (uint64_t)(((uint64_t)(((tmp)&0xfffffffful))<<32));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_for_for_loop_add_entry.val._bitmap |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_for_for_loop_add_entry(p, tempdata.module_for_for_loop_add_entry.key, tempdata.module_for_for_loop_add_entry.val);
      } break;
    default:
