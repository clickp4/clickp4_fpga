...
int MainRequest_module_firewall_firewall_with_tcp_add_entry ( struct PortalInternal *p, const ModuleFirewallFirewallWithTcpReqT key, const ModuleFirewallFirewallWithTcpRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_firewall_firewall_with_tcp_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_firewall_firewall_with_tcp_add_entry, "MainRequest_module_firewall_firewall_with_tcp_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.src_addr>>30)|(((unsigned long)key.padding)<<2));
    p->transport->write(p, &temp_working_addr, (key.dst_addr>>30)|(((unsigned long)key.src_addr)<<2));
    p->transport->write(p, &temp_working_addr, (key.src_port>>14)|(((unsigned long)key.dst_addr)<<2));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.dst_port)<<2)|(((unsigned long)key.src_port)<<18));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_firewall_firewall_with_tcp_add_entry << 16) | 5, -1);
    return 0;
};

int MainRequest_module_firewall_firewall_with_udp_add_entry ( struct PortalInternal *p, const ModuleFirewallFirewallWithUdpReqT key, const ModuleFirewallFirewallWithUdpRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_firewall_firewall_with_udp_add_entry, 5);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_firewall_firewall_with_udp_add_entry, "MainRequest_module_firewall_firewall_with_udp_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.src_addr>>30)|(((unsigned long)key.padding)<<2));
    p->transport->write(p, &temp_working_addr, (key.dst_addr>>30)|(((unsigned long)key.src_addr)<<2));
    p->transport->write(p, &temp_working_addr, (key.src_port>>14)|(((unsigned long)key.dst_addr)<<2));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.dst_port)<<2)|(((unsigned long)key.src_port)<<18));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_firewall_firewall_with_udp_add_entry << 16) | 5, -1);
    return 0;
};
...
MainRequestCb MainRequestProxyReq = {
    ...
    MainRequest_module_firewall_firewall_with_tcp_add_entry,
    MainRequest_module_firewall_firewall_with_udp_add_entry,
};
MainRequestCb *pMainRequestProxyReq = &MainRequestProxyReq;

const uint32_t MainRequest_reqinfo = 0xe0018;
const char * MainRequest_methodSignatures()
{
    return "{\"module_firewall_firewall_with_udp_add_entry\": [\"long\", \"long\"], \"read_pktcap_perf_info\": [], \"read_version\": [], \"writeMetaGenData\": [\"long\", \"long\", \"long\", \"long\"], \"metagen_start\": [\"long\", \"long\"], \"writePktGenData\": [\"long\", \"long\", \"long\", \"long\"], \"writePacketData\": [\"long\", \"long\", \"long\", \"long\"], \"pktcap_stop\": [], \"pktcap_start\": [\"long\"], \"pktgen_stop\": [], \"set_verbosity\": [\"long\"], \"pktgen_start\": [\"long\", \"long\", \"long\"], \"metagen_stop\": [], \"module_firewall_firewall_with_tcp_add_entry\": [\"long\", \"long\"]}";
}
...
int MainRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_module_firewall_firewall_with_tcp_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.src_addr = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.padding = (uint8_t)(((tmp>>2)&0x7ul));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.dst_addr = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.src_addr |= (uint32_t)(((tmp>>2)&0x3ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.src_port = (uint16_t)(((uint16_t)(((tmp)&0x3ul))<<14));
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.dst_addr |= (uint32_t)(((tmp>>2)&0x3ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_tcp_add_entry.val._action = (uint8_t)(((tmp)&0x3ul));
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.dst_port = (uint16_t)(((tmp>>2)&0xfffful));
        tempdata.module_firewall_firewall_with_tcp_add_entry.key.src_port |= (uint16_t)(((tmp>>18)&0x3ffful));((MainRequestCb *)p->cb)->module_firewall_firewall_with_tcp_add_entry(p, tempdata.module_firewall_firewall_with_tcp_add_entry.key, tempdata.module_firewall_firewall_with_tcp_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_firewall_firewall_with_udp_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 4, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_udp_add_entry.key.src_addr = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.module_firewall_firewall_with_udp_add_entry.key.padding = (uint8_t)(((tmp>>2)&0x7ul));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_udp_add_entry.key.dst_addr = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.module_firewall_firewall_with_udp_add_entry.key.src_addr |= (uint32_t)(((tmp>>2)&0x3ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_udp_add_entry.key.src_port = (uint16_t)(((uint16_t)(((tmp)&0x3ul))<<14));
        tempdata.module_firewall_firewall_with_udp_add_entry.key.dst_addr |= (uint32_t)(((tmp>>2)&0x3ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_firewall_firewall_with_udp_add_entry.val._action = (uint8_t)(((tmp)&0x3ul));
        tempdata.module_firewall_firewall_with_udp_add_entry.key.dst_port = (uint16_t)(((tmp>>2)&0xfffful));
        tempdata.module_firewall_firewall_with_udp_add_entry.key.src_port |= (uint16_t)(((tmp>>18)&0x3ffful));((MainRequestCb *)p->cb)->module_firewall_firewall_with_udp_add_entry(p, tempdata.module_firewall_firewall_with_udp_add_entry.key, tempdata.module_firewall_firewall_with_udp_add_entry.val);
      } break;
    default:
