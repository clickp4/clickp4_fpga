...
int MainRequestJson_module_l2_switch_dmac_add_entry ( struct PortalInternal *p, const ModuleL2SwitchDmacReqT key, const ModuleL2SwitchDmacRspT val )
{
    Json::Value request;
    request.append(Json::Value("module_l2_switch_dmac_add_entry"));
    Json::Value _keyValue;
    _keyValue["__type__"]="ModuleL2SwitchDmacReqT";
    ...
    _valValue["__type__"]="ModuleL2SwitchDmacRspT";
    _valValue["_action"] = (Json::UInt64)val._action;
    _valValue["_port"] = (Json::UInt64)val._port;
    request.append(_valValue);

    std::string requestjson = Json::FastWriter().write(request);;
    connectalJsonSend(p, requestjson.c_str(), (int)CHAN_NUM_MainRequest_module_l2_switch_dmac_add_entry);
    return 0;
};

int MainRequestJson_module_l2_switch_smac_add_entry ( struct PortalInternal *p, const ModuleL2SwitchSmacReqT key, const ModuleL2SwitchSmacRspT val )
{
    Json::Value request;
    request.append(Json::Value("module_l2_switch_smac_add_entry"));
    Json::Value _keyValue;
    _keyValue["__type__"]="ModuleL2SwitchSmacReqT";
    _keyValue["padding"] = (Json::UInt64)key.padding;
    _keyValue["src_addr"] = (Json::UInt64)key.src_addr;
    request.append(_keyValue);
    Json::Value _valValue;
    _valValue["__type__"]="ModuleL2SwitchSmacRspT";
    _valValue["_action"] = (Json::UInt64)val._action;
    _valValue["_receiver"] = (Json::UInt64)val._receiver;
    request.append(_valValue);

    std::string requestjson = Json::FastWriter().write(request);;
    connectalJsonSend(p, requestjson.c_str(), (int)CHAN_NUM_MainRequest_module_l2_switch_smac_add_entry);
    return 0;
};
...
MainRequestCb MainRequestJsonProxyReq = {
    ...
    MainRequestJson_module_l2_switch_dmac_add_entry,
    MainRequestJson_module_l2_switch_smac_add_entry,
};
...
int MainRequestJson_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_module_l2_switch_dmac_add_entry: {
        ((MainRequestCb *)p->cb)->module_l2_switch_dmac_add_entry(p, tempdata.module_l2_switch_dmac_add_entry.key, tempdata.module_l2_switch_dmac_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_l2_switch_smac_add_entry: {
        ((MainRequestCb *)p->cb)->module_l2_switch_smac_add_entry(p, tempdata.module_l2_switch_smac_add_entry.key, tempdata.module_l2_switch_smac_add_entry.val);
      } break;
    default:
