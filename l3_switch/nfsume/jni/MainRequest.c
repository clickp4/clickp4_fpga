...
int MainRequest_module_l3_switch_forward_table_add_entry ( struct PortalInternal *p, const ModuleL3SwitchForwardTableReqT key, const ModuleL3SwitchForwardTableRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_l3_switch_forward_table_add_entry, 4);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_l3_switch_forward_table_add_entry, "MainRequest_module_l3_switch_forward_table_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.nhop_ipv4>>6)|(((unsigned long)key.padding)<<26));
    p->transport->write(p, &temp_working_addr, (val._dmac>>23)|(((unsigned long)val._action)<<25)|(((unsigned long)key.nhop_ipv4)<<26));
    p->transport->write(p, &temp_working_addr, val._port|(((unsigned long)val._dmac)<<9));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_l3_switch_forward_table_add_entry << 16) | 4, -1);
    return 0;
};

int MainRequest_module_l3_switch_ipv4_nhop_add_entry ( struct PortalInternal *p, const ModuleL3SwitchIpv4NhopReqT key, const ModuleL3SwitchIpv4NhopRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_l3_switch_ipv4_nhop_add_entry, 4);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_l3_switch_ipv4_nhop_add_entry, "MainRequest_module_l3_switch_ipv4_nhop_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.dst_addr>>31)|(((unsigned long)key.padding)<<1));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.dst_addr)<<1));
    p->transport->write(p, &temp_working_addr, val._nhop_ipv4);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_l3_switch_ipv4_nhop_add_entry << 16) | 4, -1);
    return 0;
};

int MainRequest_module_l3_switch_send_frame_add_entry ( struct PortalInternal *p, const ModuleL3SwitchSendFrameReqT key, const ModuleL3SwitchSendFrameRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_module_l3_switch_send_frame_add_entry, 3);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_module_l3_switch_send_frame_add_entry, "MainRequest_module_l3_switch_send_frame_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (val._smac>>32)|(((unsigned long)val._action)<<16)|(((unsigned long)key.egress_spec)<<17));
    p->transport->write(p, &temp_working_addr, val._smac);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_module_l3_switch_send_frame_add_entry << 16) | 3, -1);
    return 0;
};
...
MainRequestCb MainRequestProxyReq = {
    ...
    MainRequest_module_l3_switch_forward_table_add_entry,
    MainRequest_module_l3_switch_ipv4_nhop_add_entry,
    MainRequest_module_l3_switch_send_frame_add_entry,
};
...
int MainRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_module_l3_switch_forward_table_add_entry: {
        ...
        p->transport->recv(p, temp_working_addr, 3, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_forward_table_add_entry.key.nhop_ipv4 = (uint32_t)(((uint32_t)(((tmp)&0x3fffffful))<<6));
        tempdata.module_l3_switch_forward_table_add_entry.key.padding = (uint8_t)(((tmp>>26)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_forward_table_add_entry.val._dmac = (uint64_t)(((uint64_t)(((tmp)&0x1fffffful))<<23));
        tempdata.module_l3_switch_forward_table_add_entry.val._action = (uint8_t)(((tmp>>25)&0x1ul));
        tempdata.module_l3_switch_forward_table_add_entry.key.nhop_ipv4 |= (uint32_t)(((tmp>>26)&0x3ful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_forward_table_add_entry.val._port = (uint16_t)(((tmp)&0x1fful));
        tempdata.module_l3_switch_forward_table_add_entry.val._dmac |= (uint64_t)(((tmp>>9)&0x7ffffful));((MainRequestCb *)p->cb)->module_l3_switch_forward_table_add_entry(p, tempdata.module_l3_switch_forward_table_add_entry.key, tempdata.module_l3_switch_forward_table_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_l3_switch_ipv4_nhop_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 3, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_ipv4_nhop_add_entry.key.dst_addr = (uint32_t)(((uint32_t)(((tmp)&0x1ul))<<31));
        tempdata.module_l3_switch_ipv4_nhop_add_entry.key.padding = (uint8_t)(((tmp>>1)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_ipv4_nhop_add_entry.val._action = (uint8_t)(((tmp)&0x1ul));
        tempdata.module_l3_switch_ipv4_nhop_add_entry.key.dst_addr |= (uint32_t)(((tmp>>1)&0x7ffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_ipv4_nhop_add_entry.val._nhop_ipv4 = (uint32_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_l3_switch_ipv4_nhop_add_entry(p, tempdata.module_l3_switch_ipv4_nhop_add_entry.key, tempdata.module_l3_switch_ipv4_nhop_add_entry.val);
      } break;
    case CHAN_NUM_MainRequest_module_l3_switch_send_frame_add_entry: {
        
        p->transport->recv(p, temp_working_addr, 2, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_send_frame_add_entry.val._smac = (uint64_t)(((uint64_t)(((tmp)&0xfffful))<<32));
        tempdata.module_l3_switch_send_frame_add_entry.val._action = (uint8_t)(((tmp>>16)&0x1ul));
        tempdata.module_l3_switch_send_frame_add_entry.key.egress_spec = (uint16_t)(((tmp>>17)&0x1fful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.module_l3_switch_send_frame_add_entry.val._smac |= (uint64_t)(((tmp)&0xfffffffful));((MainRequestCb *)p->cb)->module_l3_switch_send_frame_add_entry(p, tempdata.module_l3_switch_send_frame_add_entry.key, tempdata.module_l3_switch_send_frame_add_entry.val);
      } break;
    default:
